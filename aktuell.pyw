...
def check_for_update_and_restart():
    try:
        url = "https://raw.githubusercontent.com/grossCalvin/Seabot/refs/heads/main/version.txt"
        response = requests.get(url, timeout=5)
        if response.status_code == 200:
            remote_version = response.text.strip()
            if remote_version != CURRENT_VERSION:
                print(f"Neue Version verfügbar: {remote_version} (aktuell: {CURRENT_VERSION})")
                # Optional: Download neues Skript hier oder nur Hinweis anzeigen
                messagebox.showinfo("Update verfügbar", f"Eine neue Version {remote_version} ist verfügbar.\nDas Programm wird neu gestartet, sobald alles aktualisiert ist.")
                return True
    except Exception as e:
        print(f"Fehler bei der Updateprüfung: {e}")
    return False
...
def check_requirements_and_restart():
    try:
        ...
        if response.status_code == 200:
            ...
            # Mapping PyPI-Paket → tatsächlicher Importname
            import_name_map = {
                "opencv-python": "cv2",
                "pillow": "PIL",
                "pyautogui": "pyautogui",
            ...
            print("Konnte requirements.txt nicht abrufen.")
    except Exception as e:
...
if __name__ == "__main__":
    update_needed = check_for_update_and_restart()
    requirements_changed = check_requirements_and_restart() 
    if update_needed or requirements_changed:
        restart_script()
    root = tk.Tk()
